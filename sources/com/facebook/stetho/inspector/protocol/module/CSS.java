...
public class CSS implements ChromeDevtoolsDomain {
        ...
        this.mDocument.postAndWait((Runnable) new Runnable() {
            public void run() {
                ...
                if (elementForNodeId == null) {
                    StringBuilder h = C0072d.m201h("Tried to get the style of an element that does not exist, using nodeid=");
                    h.append(getComputedStyleForNodeRequest.nodeId);
    ...
    public JsonRpcResult getMatchedStylesForNode(JsonRpcPeer jsonRpcPeer, JSONObject jSONObject) {
        ...
        this.mDocument.postAndWait((Runnable) new Runnable() {
            public void run() {
                ...
                if (elementForNodeId == null) {
                    StringBuilder h = C0072d.m201h("Failed to get style of an element that does not exist, nodeid=");
                    h.append(getMatchedStylesForNodeRequest.nodeId);
    ...
    public SetPropertyTextResult setPropertyText(JsonRpcPeer jsonRpcPeer, JSONObject jSONObject) {
        ...
        if (str4 == null || !str4.contains(MessageDraft.IMAGE_DELIMITER)) {
            str2 = null;
            str = null;
        } else {
            String[] split2 = setPropertyTextRequest.text.split(MessageDraft.IMAGE_DELIMITER, 2);
