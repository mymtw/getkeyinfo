...
public final class CollectionUtils {
    ...
    public static <K, V> Map<K, V> mapOfKeyValueArrays(K[] kArr, V[] vArr) {
        ...
        if (length != length2) {
            throw new IllegalArgumentException(C0023f.m107h(66, "Key and values array lengths not equal: ", length, " != ", length2));
        } else if (length == 0) {
    ...
    public static <T> Set<T> mutableSetOfWithSize(int i) {
        return i == 0 ? new C7496d() : zzb(i, true);
    }
    ...
    private static <K, V> Map<K, V> zza(int i, boolean z) {
        return i <= 256 ? new C7494b(i) : new HashMap(i, 1.0f);
    }

    private static <T> Set<T> zzb(int i, boolean z) {
        return i <= (true != z ? 256 : 128) ? new C7496d(i) : new HashSet(i, true != z ? 1.0f : 0.75f);
    }
